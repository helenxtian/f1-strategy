...
import { ArrowLeft, Trophy, Flag, BarChart2, Clock, Cpu, ArrowRightLeft, Gauge, User, Lock, AlertCircle, Zap, Calendar, MapPin, Users, Timer, TrendingUp, Map, Cloud } from 'lucide-react'; // Removed Sparkles
...
import { fetchSpecificRaceResults, fetchAvailableSessions, fetchEventSessionSchedule, fetchSessionWeather, DetailedRaceResult, AvailableSession, EventSessionSchedule, WeatherDataPoint } from '@/lib/api';
...
const Race = () => {
  ...
  const { data: weatherData, isLoading: isLoadingWeather } = useQuery<WeatherDataPoint[]>({
    queryKey: ['sessionWeather', year, eventName, selectedSession],
    queryFn: () => {
      if (!year || !eventName || !selectedSession) throw new Error("Invalid year, event or session");
      return fetchSessionWeather(year, eventName, selectedSession);
    },
    staleTime: 1000 * 60 * 5,
    gcTime: 1000 * 60 * 15,
    enabled: !!year && !!eventName && !!selectedSession && (fetchedAvailableSessions && fetchedAvailableSessions.length > 0),
  });
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-950 via-black to-gray-950 text-white">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {isLoadingResults ? (
          ...
          <Tabs defaultValue="timing" className="mt-6">
            <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-6">
              <TabsList className="flex flex-col h-fit bg-gray-800/80 border border-gray-700 rounded-lg p-2 space-y-1">
                <TabsTrigger value="timing" className="w-full justify-start data-[state=active]:bg-red-600 data-[state=active]:text-white text-gray-300 hover:bg-gray-700/50 rounded-md px-3 py-2 text-sm transition-colors">
                  <Timer className="w-4 h-4 mr-2"/>Timing
                </TabsTrigger>
                <TabsTrigger value="car" className="w-full justify-start data-[state=active]:bg-red-600 data-[state=active]:text-white text-gray-300 hover:bg-gray-700/50 rounded-md px-3 py-2 text-sm transition-colors">
                  <Cpu className="w-4 h-4 mr-2"/>Car
                </TabsTrigger>
                <TabsTrigger value="position" className="w-full justify-start data-[state=active]:bg-red-600 data-[state=active]:text-white text-gray-300 hover:bg-gray-700/50 rounded-md px-3 py-2 text-sm transition-colors">
                  <MapPin className="w-4 h-4 mr-2"/>Position
                </TabsTrigger>
                <TabsTrigger value="weather" className="w-full justify-start data-[state=active]:bg-red-600 data-[state=active]:text-white text-gray-300 hover:bg-gray-700/50 rounded-md px-3 py-2 text-sm transition-colors">
                  <Cloud className="w-4 h-4 mr-2"/>Weather
                </TabsTrigger>
              </TabsList>

              <div>
                <TabsContent value="timing" className="pt-0 mt-0">
                  <h2 className="text-xl font-semibold mb-4 text-white">Timing</h2>
                ...
                <TabsContent value="car" className="pt-0 mt-0">
                  <h2 className="text-xl font-semibold mb-4 text-white">Car</h2>
                  {year && eventName && (
                    <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 mb-8">
                        <SpeedTraceChart year={year} event={eventName} session={selectedSession} lap={selectedLap} title="" />
                        <GearMapChart year={year} event={eventName} session={selectedSession} lap={selectedLap} />
                        <ThrottleChart year={year} event={eventName} session={selectedSession} lap={selectedLap} />
                        <BrakeChart year={year} event={eventName} session={selectedSession} lap={selectedLap} />
                        <RPMChart year={year} event={eventName} session={selectedSession} lap={selectedLap} />
                        <DRSChart year={year} event={eventName} session={selectedSession} lap={selectedLap} />
                      </div>
                    </>
                  )}
                </TabsContent>

                <TabsContent value="position" className="pt-0 mt-0">
                  <h2 className="text-xl font-semibold mb-4 text-white">Position</h2>
                  {(selectedSession === 'R' || selectedSession === 'Sprint') ? (
                    <>
                      {year && eventName && <PositionsTabContent year={year} event={eventName} session={selectedSession} />}
                      {year && eventName && <div className="mt-6"><TireStrategy year={year} event={eventName} session={selectedSession} /></div>}
                    </>
                  ) : (
                    <Card className="bg-gray-900/60 border-gray-700">
                      <CardContent className="p-6 text-gray-300">Position details are available for Race and Sprint sessions.</CardContent>
                    </Card>
                  )}
                </TabsContent>

                <TabsContent value="weather" className="pt-0 mt-0">
                  <h2 className="text-xl font-semibold mb-4 text-white">Weather</h2>
                  {isLoadingWeather ? (
                    <Skeleton className="h-40 w-full bg-gray-800/60 rounded-lg" />
                        ...
                        <Table>
                          <TableHeader className="bg-gray-800/50">
                            <TableRow className="border-gray-700">
                              ...
                              <TableHead className="text-white text-right">Air °C</TableHead>
                              <TableHead className="text-white text-right">Track °C</TableHead>
                              <TableHead className="text-white text-right">Humidity %</TableHead>
                              <TableHead className="text-white text-right">Pressure</TableHead>
                              <TableHead className="text-white text-center">Rain</TableHead>
                              <TableHead className="text-white text-right">Wind</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {weatherData.slice(-20).map((w, idx) => (
                              <TableRow key={`${w.time}-${idx}`} className="border-gray-700/50 hover:bg-gray-800/40">
                                <TableCell>{w.time ?? '-'}</TableCell>
                                <TableCell className="text-right">{w.airTemp ?? '-'}</TableCell>
                                <TableCell className="text-right">{w.trackTemp ?? '-'}</TableCell>
                                <TableCell className="text-right">{w.humidity ?? '-'}</TableCell>
                                <TableCell className="text-right">{w.pressure ?? '-'}</TableCell>
                                <TableCell className="text-center">{w.rainfall ? 'Yes' : 'No'}</TableCell>
                                <TableCell className="text-right">{w.windSpeed ?? '-'}{w.windDirection !== null && w.windDirection !== undefined ? ` / ${w.windDirection}°` : ''}</TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                        </Table>
                    ...
                    <Card className="bg-gray-900/60 border-gray-700">
                      <CardContent className="p-6 text-gray-300">No weather samples available for this session.</CardContent>
                    </Card>
                  )}
                </TabsContent>
              </div>
