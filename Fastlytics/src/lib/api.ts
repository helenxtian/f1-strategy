...
export const fetchDriverStandings = async (year: number): Promise<DriverStanding[]> => {
    ...
    try {
        ...
        if (!Array.isArray(data)) {
            throw new Error(typeof data?.error === 'string' ? data.error : 'Invalid driver standings response format');
        }
...
export const fetchTeamStandings = async (year: number): Promise<TeamStanding[]> => {
    ...
    try {
        ...
        const data = await response.json();
        if (!Array.isArray(data)) {
            throw new Error(typeof data?.error === 'string' ? data.error : 'Invalid team standings response format');
        }
...
export const fetchRaceResults = async (year: number): Promise<RaceResult[]> => {
    ...
    try {
        ...
        const data = await response.json();
        if (!Array.isArray(data)) {
            throw new Error(typeof data?.error === 'string' ? data.error : 'Invalid race results response format');
        }
...
export const fetchSchedule = async (year: number): Promise<ScheduleEvent[]> => {
    ...
    try {
        ...
        const data = await response.json();
        if (!Array.isArray(data)) {
            throw new Error(typeof data?.error === 'string' ? data.error : 'Invalid schedule response format');
        }
...
/** Fetches weather telemetry samples for a session. */
export const fetchSessionWeather = async (year: number, event: string, session: string): Promise<WeatherDataPoint[]> => {
    const params = new URLSearchParams({ year: year.toString(), event, session });
    ...
    try {
        ...
        const data = await response.json();
        if (!Array.isArray(data)) {
            throw new Error(typeof data?.error === 'string' ? data.error : 'Invalid weather response format');
        }
